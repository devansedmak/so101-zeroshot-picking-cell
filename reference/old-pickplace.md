...
- **GroundingDINO prompting** — prompt construction, confidence thresholds, bbox postprocess. Port as `src/perception/grounding.py`. Re-tune thresholds on real overhead frames (lighting ≠ sim).
- **Command → structured task schema** (object, target location) — maps directly to the order payload `{"item", "bin"}`.

## ❌ Do NOT reuse

- **IK / motion code** — Franka is 7-DOF torque-controlled; SO-101 is 5-DOF + gripper with hobby servos. Write SO-101-specific `src/control/` (see decisions.md O2; follow nl_arm_controller clamp+ramp pattern).
