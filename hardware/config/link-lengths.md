...
these four numbers. Wrong lengths mean the gripper reaches short/long by that error.
...
- The base-pan pivot (J1) is assumed coaxial with the shoulder column, so no length needed.
...
2. Re-run the suite; the FK∘IK round-trip test must stay green with the new numbers:
   ...
   ```
   (If `Unreachable` starts firing for demo table points, the reach envelope shrank, so move the mat
