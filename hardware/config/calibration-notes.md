...
1. Driver auto-detects on boot: `SO101 ports: leader=/dev/ttyACM0, follower=/dev/ttyACM1`
   (by **voltage detect** — matches our by-serial map in `ports.md`).
...
5. On success: torque enables, follower **snaps to home** (abrupt, no ramp — normal).
...
- ❌ **wrist_roll (ID 5) wrapped** → recorded `[0, 4090]` "not physically possible". Cause:
  ...
  Live logs show each joint's span — watch wrist_roll is a *partial* number, not ~4090.
