...
## Bring-up runbook (~15 min, mechanical — run it in this order)
...
the arm base**, long edge pointing away from the arm — then the homography world frame IS
...
It must not move again after step 4 — if it gets bumped, redo steps 4–7. (D5/D11.)
...
**Step 7 — survey the bin pixel rectangles (this is what closes the loop, D9).**
```bash
...
```
Then dry-run the loop in **simulation** before any live motion (see runbook.md).
