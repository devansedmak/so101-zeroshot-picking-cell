...
| A real platform alert | ⬜ **never created** | The alert chain was exercised only with `--dry-run-alerts` (deliberately, to keep test noise out of the live alerts view). No `order_fulfilled` / `order_failed` alert has ever actually reached the platform. |
