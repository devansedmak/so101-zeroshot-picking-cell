...
| Bin reachability | 🟡 **diagnosed, fix computed, not yet applied** | The three zone centres sit at **r = 259.0 / 262.1 / 246.4 mm** from the pan axis against a vertical-gripper band of **171.1 – 246.6 mm**, so only zone C reaches, by 0.2 mm. [`tools/check_reach.py`](tools/check_reach.py) reproduces this and solves it: **sliding the mat 3.0 cm toward the base** puts all three at 233.8 / 232.2 / 217.2 mm with ≥ **12.8 mm** margin. The slide is aimed at the *intersection* of two kinematic models — `ik.py` assumes the shoulder pivot is coaxial with the pan axis, and it is not (30.4 mm forward, 18.3 mm lateral), which is worth ~31 mm of band — so the fix survives that correction landing later. Needs 60 s of hardware access plus a re-run of `pick_bin_regions.py`. |
