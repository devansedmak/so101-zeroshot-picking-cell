...
Real SDK surface (verified against cyberwave.alerts.TwinAlertManager.create in S5):
    ...
    cloud, workflow}; category ∈ {technical, business}. (NB: there is no ``message``
    kwarg; details go in ``description``.)
...
not part of the pure payload. The default is "simulation" (our only entrypoint today;
pass "edge" once the loop runs on real hardware).

The closed-loop visual verification that gates these alerts is the project thesis.
With a verifier wired in, "fulfilled" means the VLM saw the item in
...
def build_failed_alert(order: Order, reason: str) -> dict[str, str]:
    ...
    after pick+place+verify failed twice, a human has to look, hence
    ``severity="error"``. ``reason`` is the verifier's verdict text.
    """
    return {
...
def send_alert(
    ...
    *,
    ...
    ``alerts.create(**kwargs)``, so a fake satisfies it offline. ``source_type`` tags
