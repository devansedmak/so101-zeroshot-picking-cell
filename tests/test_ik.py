...
def test_gripper_is_held_vertical():
    ...
    #
    # ⚠ The MINUS on q4 is not a typo and must not be "tidied" back to a plus.
...
def test_too_near_target_raises_unreachable():
    # Tip height that puts the wrist exactly ON the shoulder ⇒ reach 0 < |L1-L2|.
...
def test_gripper_constants_match_the_measured_hardware_convention():
    """Pins open/closed to the follower's own calibrated 0°→128.9° gripper span.
...
def test_in_limits_reports_without_clamping():
    ...
    # calibrated gripper span is 0°→128.9° (0 = shut), so a negative "open" is not a
